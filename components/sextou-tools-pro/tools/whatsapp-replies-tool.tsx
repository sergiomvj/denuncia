...
interface WhatsAppRepliesToolProps {
...
function extractResultFromGeneration(generation: any): GeneratedReplyResult {
  const output = generation.outputData as Record<string, string | string[]>

  return {
    id: generation.id,
    title: generation.title,
    shortReply: String(output.shortReply || ""),
    fullReply: String(output.fullReply || ""),
    warmerReply: String(output.warmerReply || ""),
    firmerReply: String(output.firmerReply || ""),
    ctaReply: String(output.ctaReply || ""),
...
function buildHistoryItem(generation: any): HistoryItem {
  return {
    ...
    subtitle:
      ...
      "shortReply" in generation.outputData
        ? String(generation.outputData.shortReply)
        : undefined,
...
export function WhatsAppRepliesTool({ initialHistory }: WhatsAppRepliesToolProps) {
  const [form, setForm] = useState(initialForm)
  const [history, setHistory] = useState(initialHistory)
  const [result, setResult] = useState<GeneratedReplyResult | null>(null)
  const [selectedGenerationId, setSelectedGenerationId] = useState<string | null>(null)
  const [statusMessage, setStatusMessage] = useState(
    "Preencha os campos e gere uma resposta comercial pronta para copiar."
  )
  ...
  function validateForm() {
    ...
    if (!form.caseDetails.trim()) {
      setStatusMessage("Revise os campos destacados: descreva o contexto da conversa.")
      return false
    }

    if (!form.desiredCta.trim()) {
      setStatusMessage("Revise os campos destacados: informe a CTA desejada.")
      return false
    }
  ...
  async function generateReply(sourceAction: "GENERATE" | "REGENERATE", sourceGenerationId?: string) {
    const response = await fetch("/api/sextou-tools-pro/generate", {
      ...
      body: JSON.stringify({
        appId: "respostas-prontas-whatsapp",
        input: form,
        sourceAction,
        sourceGenerationId,
      }),
      ...
      if (response.status === 429) {
        throw new Error(
          payload?.code === "regeneration-limit-reached"
            ? "Voce ja usou as 2 regeneracoes deste resultado."
  ...
  async function handleGenerate() {
    ...
    setStatusMessage("Criando sua resposta comercial... isso costuma levar poucos segundos.")

    try {
      const generation = await generateReply("GENERATE")
      ...
      setStatusMessage("Resposta criada e salva no historico da sua conta.")
    } catch (error) {
  ...
  async function handleRegenerate() {
    if (!selectedGenerationId) {
      setStatusMessage("Gere uma resposta primeiro para usar a regeneracao.")
      return
    }

    setIsSubmitting(true)
    setStatusMessage("Criando uma nova versao da resposta...")

    try {
      const generation = await generateReply("REGENERATE", selectedGenerationId)
      const parsedResult = extractResultFromGeneration(generation)
      setResult(parsedResult)
      setSelectedGenerationId(generation.id)
      setHistory((current) => [buildHistoryItem(generation), ...current].slice(0, 8))
  ...
  async function handleCopy(text: string, label: string) {
    ...
    setStatusMessage(`${label} copiada para a area de transferencia.`)
  }
  ...
  async function handleGenerationAction(action: "favorite" | "unfavorite" | "archive" | "restore" | "delete") {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Nao foi possivel atualizar esse resultado agora.")
      }
      ...
      if (action === "favorite") setStatusMessage("Resultado salvo como favorito/template.")
      if (action === "unfavorite") setStatusMessage("Resultado removido dos favoritos.")
      if (action === "archive") setStatusMessage("Resultado arquivado.")
      if (action === "restore") setStatusMessage("Resultado restaurado.")
      if (action === "delete") setStatusMessage("Resultado removido da sua biblioteca ativa.")
    } catch (error) {
      setStatusMessage(error instanceof Error ? error.message : "Nao foi possivel atualizar o resultado.")
    } finally {
  ...
  async function handleDuplicate() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Nao foi possivel duplicar esse resultado agora.")
      }

      setHistory((current) => [buildHistoryItem(payload.generation), ...current].slice(0, 8))
      setStatusMessage("Resultado duplicado no historico.")
    } catch (error) {
  ...
  async function handlePostActionCopy(kind: "followup" | "faq") {
    if (!result) {
      return
    }
    ...
    const text =
      kind === "followup"
        ? `Crie uma sequencia curta de follow-up com base nesta resposta principal:\n\n${result.fullReply}`
        : `Crie um FAQ curto com base nesta resposta principal:\n\n${result.fullReply}`

    await navigator.clipboard.writeText(text)
    setStatusMessage(
      kind === "followup"
        ? "Brief de follow-up copiado. Use quando o app de follow-up entrar no ar."
        : "Brief de FAQ copiado. Use quando o app de FAQ entrar no ar."
    )
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_minmax(0,0.9fr)]">
      <section className="rounded-[24px] border border-white/10 bg-[#171717] p-6">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <label className={labelClass}>Situacao da resposta</label>
            <select
              className={fieldClass}
          ...
          <div className="md:col-span-2">
            <label className={labelClass}>Contexto da conversa</label>
            <textarea
              className={textareaClass}
              value={form.caseDetails}
              onChange={(event) => updateField("caseDetails", event.target.value)}
              placeholder="Explique o caso com os detalhes que voce normalmente mandaria em um audio ou texto."
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex.: me chama para fechar hoje"
            />
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          >
            {isSubmitting ? "Gerando..." : "Gerar respostas"}
          </button>
          <button
            type="button"
      ...
      <aside className="space-y-6">
        <div className="rounded-[24px] border border-white/10 bg-[#171717] p-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 font-toolkit text-2xl font-extrabold text-[#F0EDE6]">
                {result ? result.title : "Sua resposta aparece aqui"}
              </h2>
            </div>
            {result ? (
          ...
          {result ? (
            <div className="mt-4 space-y-3">
              {[
                { label: "Resposta curta", value: result.shortReply },
                { label: "Resposta completa", value: result.fullReply },
                { label: "Versao mais simpatica", value: result.warmerReply },
                { label: "Versao mais firme", value: result.firmerReply },
                { label: "Versao com CTA", value: result.ctaReply },
              ].map((item) => (
            ...
            <div className="mt-4 rounded-[22px] border border-white/10 bg-white/5 p-4 text-sm leading-7 text-[#A09D97]">
              Voce ainda nao criou nenhum resultado neste app. Preencha o formulario e gere sua
          ...
          <div className="mt-4 space-y-3">
            {history.length === 0 ? (
              <div className="rounded-[22px] border border-white/10 bg-white/5 p-4 text-sm leading-7 text-[#A09D97]">
                Seu historico deste app vai aparecer aqui conforme voce gerar novas respostas.
              </div>
            ) : (
              history.map((item) => (
                <div key={item.id} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      {item.subtitle ? (
                        <p className="mt-1 text-xs text-[#A09D97] line-clamp-2">{item.subtitle}</p>
                      ) : null}
                    </div>
        ...
        <div className="rounded-[24px] border border-white/10 bg-[#171717] p-6 text-sm leading-7 text-[#A09D97]">
          ...
          <p className="mt-4">
            O foco aqui e acelerar atendimento comercial. Revise a resposta antes de enviar quando o
            caso envolver conflito, cobranca delicada ou reclamacao sensivel.
          </p>
        </div>
      </aside>
    </div>
